...
tags:
  ...
  - text_mining
  - twitter
slug: content/post/2016-12-21-r-twitter
---
...
```{r, include=FALSE}
consumer_key <- "6fDhPiSfgUfhR6PY6Jbs5Q"
consumer_secret <- "PTtmEaEg4cZwiVwZDs9oAvguySYL9sUH5G6rNOUSDSc"
access_token <- "15845089-kCromuoCKRwXHtW5BOCjE4GtnDNtX2i8ol0ZeTbHU"
access_secret <- "owu7Z0RMKV9bskxUXAE8qFiyyTZuTZQaoFCXEeweTs"
```
...
```{r, echo=TRUE}
library(dplyr)
gvl_twitter_unique <- gvl_twitter_df %>% 
  filter(!isRetweet)
...
setup_twitter_oauth(consumer_key, consumer_secret, access_token, access_secret) # needed to knit the Rmd file, may not be necessary for you to reauthenticate in 1 session
    options(httr_oauth_cache=origop)
gvl_twitter_unique <- searchTwitter("#yeahthatgreenville",n=200) %>% 
  twListToDF()
 
gvl_twitter_nolink <- gvl_twitter_unique %>% 
  mutate(text=gsub("https?://[\\w\\./]+","",text,perl=TRUE))
```
...
tweet_words <- gvl_twitter_nolink %>% 
  select(id,text) %>% 
...
my_stop_words <- stop_words %>% 
  select(-lexicon) %>% 
  bind_rows(data.frame(word=c("https","t.co","yeahthatgreenville","amp","gvl"))) 
```
...
```{r, echo=TRUE}
gvl_sentiment <- tweet_words_interesting %>% 
  left_join(bing_lex)
...
On average, there are `r gvl_sent_anly2[gvl_sent_anly2$sentiment=="positive" & !is.na(gvl_sent_anly2$sentiment),"n"]` positive words per tweet and `r gvl_sent_anly2[gvl_sent_anly2$sentiment=="negative" & !is.na(gvl_sent_anly2$sentiment),"n"]` negative words per tweet, if you accept the assumptions of the above analysis.
